...
const priorityItems = [
    { title: "فوری", icon: icons.redFlag },
    { title: "بالا", icon: icons.yellowFlag },
    { title: "متوسط", icon: icons.greenFlag },
    { title: "پایین", icon: icons.grayFlag },
    { title: "حذف اولویت", icon: icons.deletePriority },
...
export const NewTask = () => {
    ...
    const [editTag, setEditTag] = useState({flag : false, id : null});
    ...
    function handleSubmitInputChange(event){
        ...
        if (event.key === 'Enter') {
            const newItem = { id: uuidv4(), title: text, color: "#93fa7f", editFlag : false };
            // console.log(newItem)
            tag.push(newItem);
            // console.log(tag)
            setTag([...tag]);
    ...
    return (
        ...
        >   
            ...
            <Card title="" className="flex p-0 flex-col items-end py-[36px] px-[44px] gap-[40px] w-[1166px] h-[576px] shadow-[0_4px_16px_0_rgba(0,0,0,0.16)] rounded-[20px] pt-[36px] ">
                {/* Frame 184 */}
                ...
                <div className='Frame-185 flex flex-row justify-start items-center gap-[8px] w-[1078px] h-[34px] '>
                    <span className=' not-italic font-medium text-[16px] leading-[25px] text-right text-[#000000]'>در</span>
                    <div className='box-border flex flex-row justify-start items-center pt-[5px] px-[8px] pb-[4px] gap-[10px]  h-[33px] border border-[#E9EBF0] rounded-[6px]'>
                        ...
                        <input className='not-italic w-[158px] font-normal text-[16px] leading-[24px] text-right text-[#1E1E1E]' placeholder='عنوان تسک' />

                    </div>
                    <span>برای</span>
                    <i>
                        {icons.membersTaskIcon}
                    </i>


                </div>
                {/* Frame 186 */}
                <textarea placeholder='توضیحاتی برای این تسک بنویسید' className='py-[19px] px-[21px] resize-none border-box w-[1078px] h-[191px] border border-[#E2E2E2] rounded-[12px]' />

                {/* Frame 188 */}
                <div className='cursor-pointer flex flex-row justify-start items-center gap-[15px] w-[1078px] h-[32px]'>
                    <span className='not-italic font-normal tex-[16px] leading-[24px] text-right'>افزودن پیوست </span>
                ...
                <div className='flex flex-row justify-between  items-center gap-[670px] w-[1078px] h-[50px]'>
                    <div className='flex flex-row justify-start items-center gap-[24px] w-[272px] h-[50px]'>
                        <div className='cursor-pointer' onClick={handleFlagOpen}><i>{icons.priorityTaskIcon}</i>
                        ...
                        <Transition
                                ...
                                leaveTo="opacity-0"
                                className=" absolute">
                                <Card className={`absolute mt-[-220px]  w-[166px] justify-start rounded-[8px]  gap-[16px] p-[12px] shadow-xl`}>
                        ...
                        <div className='cursor-pointer' onClick={handleDatePickerOpen}><i>{icons.CalendarTaskIcon}</i></div>
                        <div className='cursor-pointer' onClick={handleTagOpen}><i>{icons.tagTaskIcon}</i>
                        ...
                        <Transition
                            ...
                            <div className='flex flex-col items-start gap-[4px] w-[153px] h-[60px]'>
                                ...
                                {tag.map((item) => (
                                     <div key={item.id} ref={tagHover} id={item.id}   onMouseEnter={() => handleTagEditHover(item.id)} onMouseLeave={() => handleTagEditHoverLeave(item.id)} className='flex cursor-pointer flex-row justify-start items-center py-[5px] px-[8px] gap-[8px] w-[54px] h-[24px] bg-[#FFAE34] rounded-[6px]'>
                                     {!item.editFlag  ? <span className=' not-italic font-normal text-[10px] leading-[15px] text-right text-[#1E1E1E]'>{item.title}</span> : <div className=' w-[54px] flex flex-row justify-between items-center'>
                                            <i onClick={() => handleClickCloseTag(item)}>{icons.CloseTagIcon}</i>
                                ...
                                <div className='flex flex-row justify-start items-center py-[4px] px-[8px] gap-[8px] w-[153px] h-[32px] bg-[#E9E9E9] rounded-[4px]'>
                                    ...
                                    <input className='w-[103px] h-[18px] not-italic font-normal text-[12px] leading-[18px] text-[#534D60] bg-[#E9E9E9]' type="text" onChange={handleTagInputChange} onKeyDown={handleSubmitInputChange} name="" id="" value={tagInputText} placeholder='جستجو یا ساختن تگ' />
                            ...
                            {tagItems.map((item) => (
                                <div className='flex flex-col justify-center items-start w-[153px]  mt-[12px]'>
                                <div className='flex flex-row justify-between items-center gap-[12px] w-[153px] h-[31px]'>
                                    <div style={{backgroundColor : `${item.color}`  }} className='flex flex-row justify-start items-center px-[5px] py-[8px] gap-[8px] rounded-[6px]'>
                                        <span className='not-italic font-normal text-[14px] leading-[21px] text-right text-[#1E1E1E]'>{item.title}</span>
